/*
   ...
   referdev_v7 -- devkit-c esp32
      --moving to sourcetree
      renamed to in vscode
      again
      once more
...
// LED Pin
const int LED_PIN = 2; //GPIO 2 on these Hiletgo boards
...
void setup() {
  ...
  for (int i = 0; i < sizeof(referDutycyclearray) / sizeof(referDutycyclearray[0]); i += 2 ) { //   Should be 50% as initialized.
    referDutycyclearray[i] = 1;

  }
  memset(freezerDutycyclearray, 0, sizeof(freezerDutycyclearray));
  for (int i = 0; i < sizeof(freezerDutycyclearray) / sizeof(freezerDutycyclearray[0]); i += 2 ) { //   Should be 50% as initialized.
...
void getTemps() {
  ...
  if ( temp > 180 || temp < 10  ) {  // range -55 to 125 C.  retry.   Getting -127C/196.60F and sometimes zero, apparent timing after "sensors.requestTemperatures()"
  ...
  if ( temp > 180 || temp < 1  ) {  // range -55 to 125 C.  retry.   Getting -127C/196.60F, apparent timing after "sensors.requestTemperatures()"
    Serial.println("ERROR reading Temperature: ");
  ...
  if (ow.search(address))
  {
    ...
    do {
      ...
      for (uint8_t i = 0; i < 8; i++)
      {
        ...
        tcpClient.print("0x");
        if (address[i] < 0x10) tcpClient.print("0");
        tcpClient.print(address[i], HEX);
        if (i < 7) tcpClient.print(", ");
      }
...
void coolingControl() { //<------------------------------------------------------COOLING CONTROL
  tcpClient.print(millis());
  ...
  if (referTemperatureF > referSetpointF + 1 && referCvalue == 0 ) {
    ...
    if (referCvalue != 0 ) {  //Compressor is ON for remaining function.
      if (referTemperatureF > referSetpointF + referSetpointoffsetF) {  //too hot, full cooling
  ...
  if (freezerTemperatureF > freezerSetpointF + 1 && freezerCvalue == 0 ) {
    ...
    if (freezerCvalue != 0 ) {  //Compressor is ON for remaining function.
      if (freezerTemperatureF > freezerSetpointF + freezerSetpointoffsetF) {  //too hot, full cooling
...
void adjustCeiling() {  //do this every N minutes, default 5.
  tcpClient.print(millis());
  ...
  if ( referDutycycle <= maxDutycycle ) {
    ...
    else {  //case under duty cycle, ceiling >200
      referCceiling = 255;
    }
  }
  else {   //case for block, over duty cycle
    if (referCceiling > referCfloor && referCvalue != 0) {
      ...
      else {  //refer over duty cycle, on  and Ceiling !>= 92+55
        referCceiling = referCfloor;
        referCvalue = referCfloor;
  ...
  if ( freezerDutycycle <= maxDutycycle ) {
    ...
    if (freezerCceiling > freezerCfloor && freezerCvalue != 0) {
      ...
      else {  //freezer over duty cycle, on  and Ceiling !>= 92+55
        freezerCceiling = freezerCfloor;
        freezerCvalue = freezerCfloor;
        // Serial.println("setting all values to floor (92), max speed");
      }
...
int calcDutycycle () {  //int for value returning function   Try passing the array to do it for freezer and refer.
...
void networkcheck() {
  ...
  switch (conn_stat) {
    ...
    case 1:                                                       // WiFi starting, do nothing here
      ...
      // suspect it will hang here on reconnect
      waitCount++;
      break;
...
void loop() {
  ...
  if (conn_stat == 5) {
    if (millis() - lastMsg > 15000) {                            // Start send status every n sec (just as an example)
  ...
  // Start cooling logic
  if ( millis() - lastCoolingAdjustment > coolingPeriod * 1000UL ) {  //in seconds default 60
    coolingControl();
  ...
  if ( millis() - lastCeilingAdjustment > ceilingPeriod * 60UL * 1000UL ) { //in Minutes   default 5.
